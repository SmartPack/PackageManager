 ...
 */
public class SplitAPKInstallerNoRoot {
    ...
    public static void installSplitAPKs(Activity activity) {
        new AsyncTask<Void, Void, Void>() {
            ...
            protected void onPreExecute() {
                super.onPreExecute();
                SplitAPKInstaller.mInstall = true;
                ...
                Utils.saveString("installationStatus", null, activity);
                Utils.saveString("errorStatus", null, activity);
            ...
            protected Void doInBackground(Void... voids) {
                ...
                PackageTasks.mOutput.append(" - ").append(activity.getString(R.string.session_id, String.valueOf(sessionId))).append("\n\n");
                    ...
                    if (Utils.getBoolean("filePicker", true, activity)) {
                        for (String string : PackageExplorer.mAPKList) {
                            if (Utils.exist(string) && string.endsWith(".apk")) {
                                File mFile = new File(string);
                                if (mFile.exists() && mFile.getName().endsWith(".apk")) {
                                    PackageTasks.mOutput.append(" - ").append(mFile.getName()).append(": ").append(mFile.length()).append(" KB\n");
                                    runInstallWrite(mFile.length(), sessionId, mFile.getName(), mFile.toString(), activity);
                                }
                            }
                        }
                    } else {
                        for (final String splitApps : SplitAPKInstaller.splitApks(activity.getCacheDir().getPath() + "/splits")) {
                            if (splitApps.endsWith(".apk")) {
                                File mFile = new File(activity.getCacheDir().getPath() + "/splits/" + splitApps);
                                PackageTasks.mOutput.append(" - ").append(mFile.getName()).append(": ").append(mFile.length()).append(" KB\n");
                                runInstallWrite(mFile.length(), sessionId, mFile.getName(), mFile.toString(), activity);
                            }
                        }
